...
static optional<Result> try_finish_binary_upto(int built, int max_digits, long long target) {
    Alg tgt = Alg::rational(target);

    int max_total = min(max_digits, 2 * built);

    for (int total = 2; total <= max_total; total++) {
        optional<string> best_expr;

        auto consider = [&](const string& expr) {
            if (!best_expr || finish_expr_score(expr) < finish_expr_score(*best_expr)) {
                best_expr = expr;
            }
        };
        ...
        for (int i = 1; i <= built; i++) {
            int j = total - i;
            if (j < 1 || j > built) continue;
            if (sets[i].empty() || sets[j].empty()) continue;

            for (const auto& pa : sets[i]) {
                const Alg& A = pa.first;
                const Entry& EA = pa.second;

                Alg B;

                // A + B = target  =>  B = target - A
                if (sub_alg(tgt, A, B)) {
                    auto it = sets[j].find(B);
                    if (it != sets[j].end()) {
                        consider(par(EA.expr + " + " + it->second.expr));
                    }
                }
                ...
                // A / B = target  =>  B = A / target
                // target is nonzero by main() validation.
                if (!A.is_zero() && target != 0 && div_alg(A, tgt, B)) {
                    auto it = sets[j].find(B);
                    if (it != sets[j].end() && !B.is_zero()) {
                        consider(par(EA.expr + " / " + it->second.expr));
                    }
                }
            }
        ...
        if (best_expr) {
            return Result{true, total, *best_expr};
        }
    }
...
Result solve(int digit, long long target, int max_digits) {
    ...
    for (int n = 1; n <= max_digits; n++) {
        ...
        // General target-aware meet-in-the-middle finishing pass.
        // This can find target = S[i] op S[j] without explicitly constructing S[i+j].
        if (auto e = try_finish_binary_upto(n, max_digits, target)) {
            return *e;
        }
