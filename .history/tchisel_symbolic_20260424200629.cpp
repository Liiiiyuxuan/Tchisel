...
static const int MAX_SET_SIZE = 500000;
...
Result solve(int digit, long long target, int max_digits) {
    ...
    for (int n = 1; n <= max_digits; n++) {
        ...
        // Combine previous sets.
        for (int i = n / 2; i >= 1; i--) {
            int j = n - i;
            if (sets[i].empty() || sets[j].empty()) continue;

            // Snapshot keys to avoid iterator invalidation if i == n or j == n (normally not, but safe).
            vector<pair<Alg, Entry>> A, B;
            A.reserve(sets[i].size()); B.reserve(sets[j].size());
            for (auto& kv : sets[i]) A.push_back(kv);
            for (auto& kv : sets[j]) B.push_back(kv);

            for (const auto& pa : A) {
                if ((int)sets[n].size() >= MAX_SET_SIZE) break;
                const Alg& v1 = pa.first;
                const Entry& e1 = pa.second;
                for (const auto& pb : B) {
                    if ((int)sets[n].size() >= MAX_SET_SIZE) break;
        ...
        // Target-aware finishing pass. This does not add new search power in theory;
        // it just avoids building enormous S[n+1] or S[n+2] when the answer is one
        // or two simple operations away from a symbolic intermediate.
        if (n + 1 == max_digits) {
