...
Result solve(int digit, long long target, int max_digits) {
    ...
    for (int n = 1; n <= max_digits; n++) {
        // If we have a candidate with k digits, building S[k] can't help
        // (a direct find in S[k] = k digits = same). Only S[k-1] or less
        // could improve (k-1 digits). So stop when n >= candidate digits.
        ...
        if (it != sets[n].end()) {
            if (!best_result.found || n < best_result.digits) {
                best_result = {true, n, it->second.expr};
                cerr << "    [direct] found in S[" << n << "]" << endl;
            }
            return best_result;  // Can't do better than a direct find at level n
        }
        ...
        // Check finish passes — but don't return immediately.
        // Store as a candidate; keep building sets for the actual optimum.
        if (n + 1 <= max_digits) {
            if (auto e = try_finish_with_one_digit(n, target)) {
                int cand_digits = n + 1;
                if (!best_result.found || cand_digits < best_result.digits) {
                    best_result = {true, cand_digits, *e};
                    cerr << "    [finish1] candidate: " << cand_digits << " digits" << endl;
                }
            }
        }
        if (n + 2 <= max_digits) {
            if (auto e = try_finish_with_two_digits(n, target)) {
                ...
                if (!best_result.found || cand_digits < best_result.digits) {
                    best_result = {true, cand_digits, *e};
                    cerr << "    [finish2] candidate: " << cand_digits << " digits" << endl;
                }
            }
        }

        // If we already found an answer with n digits directly in S[n],
        // no point building further sets.
        if (best_result.found && best_result.digits <= n)
            return best_result;
    }
