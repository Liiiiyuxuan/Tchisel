...
Result solve(int digit, long long target, int max_digits) {
    ...
    for (int n = 1; n <= max_digits; n++) {
        ...
        // Instead of exhausting S[1]×S[8] before touching S[2]×S[7],
        // interleave: process one v1 from each partition per round.
        // This ensures every partition contributes to sets[n].

        struct Partition {
            int i, j;
            vector<pair<long long, Deriv>> vec_i;
            vector<pair<long long, Deriv>> vec_j;
        };
        ...
        // Round-robin: for each outer index, process one v1 from each partition
        // Each partition gets a fair budget of the inner loop too
        size_t max_outer = 0;
        for (auto& p : parts)
        ...
        // Track per-partition insertion counts
        ...
        for (size_t outer = 0; outer < max_outer; outer++) {
            ...
            for (int pi = 0; pi < num_parts; pi++) {
                ...
                for (auto& [v2, d2] : part.vec_j) {
                    ...
                    // Exponentiation with sqrt-reduction
