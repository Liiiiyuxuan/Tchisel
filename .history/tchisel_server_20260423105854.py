...
def find_executable() -> Path:
    candidates = [EXECUTABLE, BASE_DIR / 'solver', BASE_DIR / 'tchisel']
    for c in candidates:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        for count in range(1, max_digits + 1):
            cmd = [str(exe), str(digit), str(target), str(count)]
            proc = subprocess.run(cmd, cwd=BASE_DIR, capture_output=True, text=True, timeout=20)
            combined = ((proc.stdout or '') + '\n' + (proc.stderr or '')).strip()
            stats.append([count, 0])
            debug_runs.append({'count': count, 'stdout': proc.stdout, 'stderr': proc.stderr, 'returncode': proc.returncode})

            if proc.returncode != 0:
                continue

            formula = pick_valid_formula(proc.stdout or '', digit, target, count)
        ...
        payload = {
            ...
            'digits': max_digits,
            'stats': stats,
            'message': 'No valid formula found that both uses the selected digit and evaluates to the target.',
            'debug': debug_runs,
        }
        self._send(200, json.dumps(payload).encode(), 'application/json; charset=utf-8')
