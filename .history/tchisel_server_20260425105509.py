...
IRRATIONAL_EXECUTABLE = BASE_DIR / 'tchiselS'
...
def pick_valid_formula(text: str, digit: int, target: int, count: int):
    ...
    if not candidates:
        print(f'[validate] no formula candidates found in output')
    for cand in candidates:
        try:
            if not formula_matches_digit(cand, digit, count):
                num_tokens = re.findall(r'\d+', cand)
                total = sum(len(tok) for tok in num_tokens)
                print(f'[validate] digit check failed: need {count}x{digit}, got tokens={num_tokens} total={total}')
                continue
            value = evaluate_formula(cand)
            diff = abs(value - target)
            if diff < 0.01:  # wider tolerance for irrational intermediates
                return cand
            else:
                print(f'[validate] eval mismatch: expected {target}, got {value} (diff={diff})')
        except Exception as e:
            print(f'[validate] eval error for "{cand[:60]}...": {e}')
            continue
    return None
...
def background_solve(job_id: str, digit: int, target: int, max_digits: int, incumbent: dict):
    ...
    for solver in background_passes:
        ...
        print(f'[background] running {solver["source"]} ({solver["exe"]}) max_digits={solver["max_digits"]}')
        ...
        print(f'[background] {solver["source"]}: found={result.get("found")} digits={result.get("digits")} error={result.get("error","")} formula={str(result.get("formula",""))[:80]}')
        ...
        with _jobs_lock:
            ...
            if is_better(result, best):
                ...
                print(f'[background] ✓ {solver["source"]} improved to {result.get("digits")} digits')
            else:
                job['best'] = best
                print(f'[background] {solver["source"]} did not improve (best={best.get("digits")})')
